...
class Cart {
  ...
  cleanCategories() {
    const categoriesToClean = this.products.map(
      product => supermarketProducts.find(p => p.sku === product.sku).category
    );
    this.categories = new Set(
      [...this.categories].filter(
        category => categoriesToClean.includes(category)
      )
    );
  }
  ...
  calculateTotalPrice(products) {
    const prices = products.map(
      product => supermarketProducts.find(p => p.sku === product.sku).price
    );
    return (this.totalPrice = products.reduce(
      (acc, curr, i) => (acc += curr.quantity * prices[i]),
      0
    ));
  }
...
(async () => {
  ...
  await cart.addProduct('OL883YE', 2)
  cart
